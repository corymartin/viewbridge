...
var hoganCompileConfig = {asString: true};
...
var hoganRuntime = fs.readFileSync('node_modules/hogan.js/web/builds/2.0.0/template-2.0.0.js', 'utf8');
 ...
 *    - ext:       File extenions. Defaults are Jade:`.jade`, Hogan:`.hjs`
 * @api private
 */
...
Viewbridge.prototype = {
  ...
  _setConfig: function(config) {
    ...
    this.config.engine    = config.engine || 'jade';
    ...
    this.config.namespace = config.namespace
      ...
      : defaultNamespace;
    this.config.ext       = config.ext|| extensions[this.config.engine];
...
, _parseTemplatesForMarkers: function(callback) {
    var self = this;
    //TODO: This is shit, externalize this
    var regex = this.config.engine === 'hogan' ? HOGAN_ATTR : JADE_ATTR;
...
, outro: function() {
   ...
   */
, runtime: function() {
    if (this.config.engine === 'hogan') {
      this.comment(
        'Hogan'
      , '====='
      );
      this.buffer.push(hoganRuntime);
    }
    else {
      this.comment(
        'Jade runtime'
...
, templates: function(callback) {
    ...
    , function(view, done) {
        fs.readFile(view.path, 'utf8', function(err, tmpltext) {
          ...
          var fn;
          if (this.config.engine === 'hogan') {
            fn = hogan.compile(tmpltext, hoganCompileConfig);
          }
          else {
            fn = jade.compile(tmpltext, jadeCompileConfig);
          }
